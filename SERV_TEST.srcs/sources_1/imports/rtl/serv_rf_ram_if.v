...
module serv_rf_ram_if
  ...
  (
   ...
   generate
      if (ratio == 2) begin : gen_wtrig_ratio_eq_2
   ...
   generate
      if (width == 32) begin : gen_rreg_eq_32
         assign o_raddr = rreg;
   ...
   generate
      if (ratio == 2) begin : gen_ren_w_eq_2
         assign o_ren = rgate;
   ...
   generate
      if (ratio > 2) begin : gen_rdata1_w_neq_2
         always @(posedge i_clk) begin
   ...
   always @(posedge i_clk) begin
      if (&rcnt | i_rreq) rgate <= i_rreq;

      rtrig1 <= rtrig0;
      rcnt <= rcnt+{{CMSB{1'b0}},1'b1};
      
      if (i_rreq | i_wreq) rcnt <= {{CMSB-1{1'b0}},i_wreq,1'b0};
